...
def test_pydantic_type_coercion(monkeypatch):
    ...
    assert config.llm_config.retry.max_attempts == 5, \
        "max_attempts should be 5"

    assert isinstance(config.llm_config.retry.min_wait_seconds, int), \
        "min_wait_seconds should be int"
    assert config.llm_config.retry.min_wait_seconds == 2, \
        "min_wait_seconds should be 2"
    ...
    assert config.llm_config.cache_enabled is True, \
        "cache_enabled should be True"

    assert isinstance(config.logging.verbose, bool), \
        "verbose should be bool"
    assert config.logging.verbose is False, \
        "verbose should be False"

    assert isinstance(config.logging.format.show_time, bool), \
        "show_time should be bool"
    assert config.logging.format.show_time is True, \
        "show_time should be True (from '1')"

    assert isinstance(config.logging.levels.debug, bool), \
        "debug should be bool"
    assert config.logging.levels.debug is True, \
        "debug should be True"

    assert isinstance(config.logging.levels.info, bool), \
        "info should be bool"
    assert config.logging.levels.info is False, \
        "info should be False (from '0')"
